...
var logger = govec.InitGoVector(CONFIG.MinerID, CONFIG.MinerID, govec.GetDefaultConfig())
...
func (miner *minerlib) Start() {
	...
	var printWhenDisconnected = false
	initConnections()
	requiresUpdate = true
	for true {
		if getNumPeers() > 0 {
			printWhenDisconnected = true
			if requiresUpdate {
				updateLocalBlockChainFromPeers()
			}
			mineNextBlock()
			...
			if printWhenDisconnected {
				printWhenDisconnected = false
				fmt.Println("Miner is disconnected.")
			}
		}
	}
}
...
func (miner *minerlib) JoinPeerMiner(peerAddr string) error {
	...
	if err != nil {
		...
		logger.LogLocalEvent("Joining to Miner: "+peerAddr, govec.GetDefaultLogOptions())
...
func (miner *minerlib) PublishBlockToPeerMiner(peerAddr string, block Block, hashOfBlock string) error {
	...
	if err != nil {
		...
		logger.LogLocalEvent("Publishing Block to Miner: "+peerAddr, govec.GetDefaultLogOptions())
...
func (miner *minerlib) GetBlockChainFromPeerMiner(peerAddr string) (map[string]Block, error) {
	...
	if err != nil {
		...
		logger.LogLocalEvent("Getting Blockchain from Miner: "+peerAddr, govec.GetDefaultLogOptions())
...
func RPCClientServer() {
	...
	logger := govec.InitGoVector(CONFIG.MinerID, CONFIG.MinerID, govec.GetDefaultConfig())
...
func (t *MinerServices) CreateFile(args *NameArgs, reply *CreateReply) error {
	...
	if !exists {
		...
		opWithoutUniqueId := Operation{TimeStamp: time.Now(), Type: "CREATE", FileName: name, MinerID: CONFIG.MinerID}
...
func (t *MinerServices) AppendRec(args *RecArgs, reply *AppendReply) error {
	...
	if exists {
		...
		op := Operation{TimeStamp: time.Now(), Type: "APPEND", FileName: name, Data: args.Record, MinerID: CONFIG.MinerID}
		op, err := setOpID(op)
		if err != nil {
			fmt.Println(err.Error())
		}
	...
	for address := range getPeers() {
		if address != args.PublicAddr {
			...
			if err != nil {
				...
				logger.LogLocalEvent("Calling PublishOp on Miner: "+address, govec.GetDefaultLogOptions())
...
//func getLastSeqNumOfMinerOnLongestChain(minerId string) (int, Operation) {
//	result := 0
//	var operation Operation
//	_, longestChain, _ := getBlocksOnLongestChain()
//	//fmt.Println("getLastSeqNumOfMinerOnLongestChain", minerId, "chain length:", len(longestChain), longestChain)
//	for _, block := range longestChain {
...
//					result = op.SeqNum
//					operation = op
//					fmt.Println("last seqnum of miner on longest chain:", result, op.Id, op.Type)
//				}
//			}
//			break
//		}
